...
struct Softmax_base {
  ...
  inline __device__ Softmax_base(Params params, int tidx)
      : tidx_(tidx),
        ...
        total_blocks(0), skipped_blocks(0),
#endif
...
#pragma unroll
  ...
  inline __device__ bool compute_and_update_scale(float (&global_max)[Mma_tile_p::CORES_M],
    ...
    const bool may_skip = !IS_FIRST_COL && skip_softmax_threshold > 0;
...
#pragma unroll
    ...
    if( may_skip ) {
      ...
      skip = *((volatile uint32_t *)skip_softmax_vote);
      if( skip ) {
#ifdef SKIP_SOFTMAX_STAT
        skipped_blocks++;
#endif
...
struct Softmax<Hopper_qgmma_e4m3_fp32_traits, Kernel_traits>
  ...
  inline __device__ bool compute_and_update_scale(float (&global_max)[Mma_tile_p::CORES_M],
    ...
    const bool may_skip = !IS_FIRST_COL && this->skip_softmax_threshold > 0;
    // whether this warpgroup skips the softmax
    bool skip = may_skip;
// Row-wise max of current tile.
#pragma unroll
